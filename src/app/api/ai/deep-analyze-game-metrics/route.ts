...
import { z } from 'zod';
import { type AnalyzeChessGameOutput } from '../analyze-chess-game/route'; // Assuming route.ts exports this
...
const DeepAnalyzeGameMetricsInputSchema = z.object({
  ...
  // Add a field for the base URL of the app, so it can call its own APIs
...
const DeepAnalyzeGameMetricsOutputSchema = z.object({
  ...
  primaryWeaknesses: z.array(WeaknessSchema).min(0).max(2).describe("An array of 0 to 2 primary areas for improvement."), // min(0) for cases with no games
});
